...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock()
{
    ...
    if (m_options.test_block_validity) {
        ...
        // Fatal error
        if (!res) {
            throw std::runtime_error(strprintf("TestBlockValidity failed: %s", res.error()));
        }
...
bool SubmitBlock(ChainstateManager& chainman, const std::shared_ptr<const CBlock>& block, bool* new_block, std::string& reason, std::string& debug)
{
    ...
    bool accepted = res && res->IsValid();
    CHECK_NONFATAL(chainman.m_options.signals)->UnregisterSharedValidationInterface(sc);

    if (!res) {
        // A fatal (system) error occurred; the BlockChecked signal never fired,
        // so surface the error instead of reporting an inconclusive result.
