    ...
    try {
        ...
        if (dbp) {
            ...
            if (!res) {
                ...
                return util::Unexpected(std::move(res).error().WithMessage(
...
util::Expected<bool, kernel::FatalError> ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        if (accepted) {
            ...
            if (!res) {
                if (m_options.signals) m_options.signals->BlockChecked(block, res);
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckLevel >= 4 && !skipped_l3_checks) {
        while (pindex != chainstate.m_chain.Tip()) {
            ...
            if (auto res{chainstate.ConnectBlock(block, pindex, coins)}; !res || !res->IsValid()) {
